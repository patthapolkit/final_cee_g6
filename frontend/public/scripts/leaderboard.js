...
setInterval(async () => {
  ...
  for (const info of dataList) {
    ...
    const current_map = (await getPlayerControlbyId(playerdata.data._id)).data.currentMap;
...
function createLeaderboard(data) {
  ...
  data.forEach((player, index) => {
    ...
    listItem.textContent = `${index + 1}. ${player.playerName} - ${player.swings} swings - Map ${player.current_map}`;
